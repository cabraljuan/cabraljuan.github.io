...
\begin{itemize}
  \item {\color{teal}El numerador es lo que aumenta}
  \item {\color{teal}El denominador es lo que se entrega o cede}
\end{itemize}
...
\section*{Nota sobre bienes Giffen}



Como vimos anteriormente, los bienes Giffen son aquellos bienes para los cuales, cuando el precio aumenta, la demanda también aumenta
\[
\color{teal}
\frac{\partial Q}{\partial P} > 0
\]

Los bienes Giffen aparecen en situaciones muy particulares. Se trata de bienes inferiores que suelen ocupar una parte importante del presupuesto de consumidores con ingresos bajos. Cuando el precio de ese bien básico aumenta, el consumidor pierde poder adquisitivo y, en algunos casos extremos, termina consumiendo aún más de ese mismo bien porque no puede reemplazarlo fácilmente por otros.

Un ejemplo clásico que suele utilizarse para ilustrar este fenómeno es el caso de la papa en Irlanda. La idea general es la siguiente: para muchos hogares pobres, la papa constituía una parte muy importante de la dieta porque era un alimento relativamente accesible y que aportaba una gran cantidad de calorías. Si el precio de ese bien básico aumentaba, esos hogares sufrían una caída fuerte de su poder adquisitivo real. En ese contexto, en lugar de poder reemplazar la papa por alimentos mejores o más variados, se veían obligados a reducir todavía más el consumo de otros bienes y a destinar una proporción aún mayor de su presupuesto a la compra de papas. Esto se debe a que los otros alimentos disponibles resultaban relativamente más caros en relación con las calorías que aportaban. En otras palabras, {\color{teal}el aumento del precio no generaba una sustitución hacia otros bienes, sino una mayor dependencia del bien básico encarecido}

Este ejemplo debe interpretarse con cuidado. No se trata de que el consumidor desee más el bien porque ahora sea más caro, sino de que, al empeorar su restricción presupuestaria, queda prácticamente forzado a concentrar más su consumo en ese bien de subsistencia. Por eso, el caso de los bienes Giffen requiere condiciones muy especiales: \textbf{\color{teal}el bien debe ser inferior, debe representar una fracción importante del gasto del consumidor y, además, deben existir pocas alternativas de reemplazo cercanas}

Por esta razón, los bienes Giffen son \textbf{\color{teal}casos muy limitados y poco frecuentes en la práctica}. Su importancia es sobre todo teórica, porque muestran que la relación entre precio y cantidad demandada no siempre tiene que ser negativa

\textbf{No confundir con los bienes Veblen:} un bien Giffen no es lo mismo que un bien Veblen. En ambos casos puede ocurrir que la demanda aumente cuando sube el precio, pero la razón económica es completamente distinta. En un \textbf{\color{teal}bien Giffen}, el aumento de la demanda se explica por una situación de restricción presupuestaria severa: el consumidor compra más del bien porque no puede desplazarse hacia alternativas mejores y necesita seguir cubriendo una necesidad básica. En cambio, en un \textbf{\color{teal}bien Veblen}, el aumento de la demanda se relaciona con el carácter de lujo, prestigio o señalización social del bien. Allí el precio alto vuelve al bien más atractivo precisamente porque comunica estatus, exclusividad o poder adquisitivo. De hecho, en un bien Veblen puede pensarse que el precio alto forma parte de la naturaleza misma del bien: si el precio dejara de ser el que es, el bien cambiaría también en su valor simbólico
...
\subsection*{Otro ejemplo aún más complejo}
\[
z=z(u,v)=u^{2}v,
\qquad
u=u(p,q)=p+q,
...
En esta situación conviene graficar la relación entre variables para que quede claro:

\begin{center}
\begin{tikzpicture}[>=latex, every node/.style={draw, rounded corners}]
...
\textbf{Derivadas parciales y derivadas de las dependencias}
\[
z_u=2uv,\qquad z_v=u^{2}
\]
\[
u_p=1,\qquad u_q=1,\qquad v_q=s,\qquad v_s=q
\]
\[
p'=2,\qquad q'=2t,\qquad s'=e^{t}
\]
...
\frac{dz}{dt}
=
z_u\!\left(u_p\,p'+u_q\,q'\right)
+
z_v\!\left(v_q\,q'+v_s\,s'\right)
\]
...
\frac{dz}{dt}
=
(2uv)(1\cdot 2+1\cdot 2t)
+
(u^{2})(s\cdot 2t+q\cdot e^{t})
\]
\[
=
2uv(2+2t)+u^{2}(2ts+qe^{t})
\]

Sustituyendo \(u=p+q=2t+t^{2}\), \(v=qs=t^{2}e^{t}\), \(s=e^{t}\) y \(q=t^{2}\)
\[
\frac{dz}{dt}
=
2(2t+t^{2})(t^{2}e^{t})(2+2t)
+
(2t+t^{2})^{2}(2te^{t}+t^{2}e^{t})
\]
\[
=
2(2t+t^{2})(t^{2}e^{t})(2+2t)
...
\[
=
t^{3}e^{t}(2+t)\bigl(4(1+t)+(2+t)^{2}\bigr)
\]
\[
\color{teal}
\frac{dz}{dt}
=
e^{t}t^{3}(2+t)(t^{2}+8t+8)
\]
...
Como
\[
z(t)=u(t)^{2}v(t)=(2t+t^{2})^{2}(t^{2}e^{t})=t^{4}(2+t)^{2}e^{t}
\]

al derivar directamente
\[
\frac{d}{dt}\Big(t^{4}(2+t)^{2}e^{t}\Big)
=
e^{t}t^{3}(2+t)(t^{2}+8t+8)
\]
...
Un escalar \(\lambda\in\mathbb{R}\) es autovalor de \(A\) si existe un vector \(v\in\mathbb{R}^n\setminus\{0\}\) tal que
\[ \color{teal}
A\,v = \lambda\,v
\]
...
Una breve intuición de qué significa esto es la siguiente. Primero pensemos a la matriz A como una transformación (de la misma manera que una función al insertarle un input, cambia este input, una matriz al premultiplicar por un vector lo gira, estira, encoge, etc. En este sentido un autovector es un vector que mantiene la dirección, no gira, solamente se agranda o achica o cambia de sentido. El autovalor es la escala en la cual se agranda o achica dicho autovector. En el siguiente gráfico se puede ver la naturaleza de un autovector, en este caso sería un vector tal que tiene la dirección del vector $(1,1)$ y que la matriz al premultiplicarlo no lo gira, solamente lo achica o agranda. Dependiendo del autovalor que tenga puede achicarse o agrandarse.
...
\begin{tikzpicture}[scale=0.7]
  ...
  \draw[->] (0,-5.5) -- (0,5.5) node[above] {$x_2$};

  % Dirección del autovector (recta y = x)
  \draw[densely dashed, gray] (-5,-5) -- (5,5) node[above right] {dirección de $v$};

  % Autovector base v = (2,2)
  \draw[ultra thick] (0,0) -- (2,2);
  \fill (2,2) circle (1.2pt);
  \node[anchor=south east, xshift=-4pt, yshift=4pt] at (2,2) {$v=(2,2)$};

  % Escalados que mantienen la dirección (λ>0)
  \draw[very thick, teal]   (0,0) -- (1,1);
  \node[teal, anchor=west, xshift=8pt, yshift=-8pt] at (1,1) {$\lambda=0.5$};

  \draw[very thick, blue]   (0,0) -- (3,3);
  \node[blue, anchor=west, xshift=10pt, yshift=-10pt] at (3,3) {$\lambda=1.5$};

  \draw[very thick, purple] (0,0) -- (4,4);
  \node[purple, anchor=west, xshift=12pt, yshift=-12pt] at (4,4) {$\lambda=2$};

  % Escalados con inversión de sentido (λ<0)
  \draw[very thick, orange] (0,0) -- (-1,-1);
  \node[orange, anchor=east, xshift=-8pt, yshift=8pt] at (-1,-1) {$\lambda=-0.5$};

  \draw[very thick, red]    (0,0) -- (-2,-2);
  \node[red, anchor=east, xshift=-10pt, yshift=10pt] at (-2,-2) {$\lambda=-1$};

  \draw[very thick, brown]  (0,0) -- (-3,-3);
  \node[brown, anchor=east, xshift=-12pt, yshift=12pt] at (-3,-3) {$\lambda=-1.5$};
...
Sea \(R\in\mathbb{K}^{n\times n}\). El \emph{polinomio característico} de \(A\) se define como
\[ \color{teal}
\chi_A(\lambda)=\det\bigl(A - \lambda I\bigr)
\]
El polinomio característico proviene de la siguiente deducción, dado:
\[
A v=\lambda v,\quad v\neq 0
\]
...
Este es un sistema homogéneo con solución no trivial \(v\neq 0\) \emph{si y solo si} la matriz \(A-\lambda I\) es \textbf{singular}  
Una matriz cuadrada es singular \(\Leftrightarrow\) su determinante es cero, entonces
\[
\det(A-\lambda I)=0
\]

Los \(\lambda\) que hacen nula esa determinante son exactamente los \textbf{autovalores} de \(A\)  
Por eso se define el \emph{polinomio característico}
\[
\chi_A(\lambda)=\det(A-\lambda I)
\]
y los autovalores son las raíces de \(\chi_A\)
...
\begin{enumerate}
  \item Resolver \(\chi_A(\lambda)=0\) para obtener los autovalores \(\lambda_1,\lambda_2, \lambda_3,...\)
...
\) para \(\lambda=3\).
 \paragraph*{Observación sobre la no unicidad de los autovectores}
Los autovectores asociados a un mismo autovalor no son únicos. Es decir se pueden tomar cualquiera de los siguientes vectores como autovectores del autovalor $\lambda=1$: \((1,-1)\), \((2,-2)\), \((-3,3)\), etcétera, todos representan la misma dirección y son autovectores válidos del mismo autovalor
...
Sea \(A\in\mathbb{R}^{n\times n}\) y \(\lambda\) un autovalor de \(A\).  
\begin{itemize}
  \item La \emph{multiplicidad algebraica} de \(\lambda\), denotada \(\mathrm{mult}_{\mathrm{alg}}(\lambda)\), es el grado con que \(\lambda\) aparece como raíz del polinomio característico \(\chi_A(\lambda)\). Puede pensarse como la cantidad de veces que se repite el autovalor. 
  \[
...
Para un autovalor \(\lambda\), el espacio propio asociado a \(\lambda\) es el conjunto de todos los vectores que cumplen
\[
(A-\lambda I)v=0
\]

Ese conjunto es un subespacio vectorial. Entonces podemos preguntarnos:

\begin{center}
\emph{¿Cuántos vectores necesito para formar una base de ese espacio propio?}
\end{center}

Esa cantidad es la multiplicidad geométrica.
...
Como cada raíz aparece una sola vez en el polinomio característico, se tiene que
\[\color{teal}
\mathrm{mult}_{\mathrm{alg}}(2)=1,
\qquad
\mathrm{mult}_{\mathrm{alg}}(3)=1
\]
...
Buscamos todos los vectores \(v=(x,y)^T\) tales que
\[
(D-2I)v = 0
\]
...
Entonces \(x\) es libre, y los autovectores asociados a \(\lambda=2\) son
\[
v =
\begin{pmatrix}
x\\
...
\paragraph*{Autovectores para \(\lambda=3\)}
...
Entonces \(y\) es libre, y los autovectores asociados a \(\lambda=3\) son
\[
v =
\begin{pmatrix}
0\\
...
En este caso hay dos autovalores distintos. Cada uno tiene multiplicidad algebraica igual a 1 y multiplicidad geométrica igual a 1.

{\color{teal}
\[
\mathrm{mult}_{\mathrm{alg}}(2)=1,
\qquad
\mathrm{mult}_{\mathrm{geom}}(2)=1
\]
}
...
Primero multiplicamos las dos primeras matrices:
\[
\begin{pmatrix}x_1 & x_2 & x_3\end{pmatrix}
\begin{pmatrix}
3 & 1 & 2\\
...
Multiplicando ahora por la última matriz, obtenemos:
\[
q(x)
=
(3x_1+x_2+2x_3)x_1
...
q(x)
=
3x_1^2+x_1x_2+2x_1x_3
+
x_1x_2+4x_2^2+3x_2x_3
+
2x_1x_3+3x_2x_3+5x_3^2
\]
...
{\color{teal}
Por lo tanto, al calcular \(x^TQx\), recuperamos exactamente la forma cuadrática original}
...
La clasificación de una forma cuadrática nos dice qué forma tiene el gráfico de \(q(x)=x^TQx\).

En el caso de dos variables independientes, \(q(x_1,x_2)\), podemos dibujar su gráfico en tres dimensiones. La matriz \(Q\) determina si ese gráfico se parece a un bowl abierto hacia arriba, a un bowl dado vuelta, a un bowl achatado o a una silla de montar.

Por eso, los autovalores de \(Q\) nos dan información sobre cómo se curva el gráfico en distintas direcciones.




\begin{itemize}
  \item \textbf{Definida positiva:} el gráfico tiene forma de bowl abierto hacia arriba, como un bowl de cereales. El origen es el punto más bajo y, al movernos en cualquier dirección distinta de cero, el valor de \(q\) aumenta.

  \item \textbf{Semidefinida positiva:} el gráfico mira hacia arriba, pero puede tener alguna dirección plana. Se parece a un bowl parcialmente achatado.

  \item \textbf{Definida negativa:} el gráfico tiene forma de bowl dado vuelta, abierto hacia abajo. El origen es el punto más alto.

  \item \textbf{Semidefinida negativa:} el gráfico mira hacia abajo, pero puede tener alguna dirección plana. Es como un bowl invertido y achatado en alguna dirección.

  \item \textbf{Indefinida:} el gráfico sube en algunas direcciones y baja en otras. En dos variables independientes, la figura típica es una silla de montar.
\end{itemize}


\subsection*{Aclaración sobre dimensiones mayores}

La intuición del bowl o de la silla de montar es más fácil de visualizar con dos variables, porque el gráfico puede dibujarse en tres dimensiones.

Si la forma cuadrática tiene más variables, el gráfico estaría en más dimensiones y ya no podemos dibujarlo directamente. Sin embargo, la idea sigue siendo la misma: los autovalores indican cómo se comporta \(q(x)\) en distintas direcciones.

{\color{teal}
Si todos los autovalores son positivos, \(q(x)\) crece en todas las direcciones; si todos son negativos, decrece en todas las direcciones; si hay positivos y negativos, sube en algunas direcciones y baja en otras}


\subsection*{Ejemplo}


Planteamos una forma cuadrática homogénea de grado 2
\[
q(x_1,x_2,x_3)=2x_1^2+2x_2^2+3x_3^2+2x_1x_2
\]
...
\end{pmatrix}
\]
de modo que \(q(x)=x^\top Q x\)

Polinomio característico:
\[
\chi_Q(\lambda)=\det(Q-\lambda I)
=\det\!\begin{pmatrix}
2-\lambda & 1 & 0\\
1 & 2-\lambda & 0\\
0 & 0 & 3-\lambda
...
Factorización
\[
\chi_Q(\lambda)=(3-\lambda)\,(\lambda-1)\,(\lambda-3)
\]
...
Como \(\lambda_i>0\) para todo \(i\), concluimos que \(q\) es \textbf{definida positiva}


% HACER EXPLICACION DE FORMAS CUADRATICAS RESTRINGIDAS


\subsection*{Formas cuadráticas restringidas}



Una forma cuadrática \(q(x)=x^T A x\) asigna a cada vector \(x\) un número real. Estudiar su signo significa preguntar si ese número es siempre positivo, siempre negativo, o si puede cambiar de signo según la dirección elegida.

Cuando aparecen restricciones lineales, ya no estudiamos la forma cuadrática en todo el espacio, sino solamente sobre los vectores que cumplen esas restricciones. Es decir, si las restricciones se escriben como
\[
C x = 0
\]
entonces solo nos interesan los vectores \(x\) que satisfacen ese sistema de ecuaciones.

La idea central es que las restricciones reducen las direcciones posibles. Por eso, una forma cuadrática que no es positiva definida en todo el espacio podría ser positiva definida si la miramos solamente sobre las direcciones permitidas por las restricciones.

Para estudiar una forma cuadrática restringida, hay dos métodos equivalentes:

\begin{itemize}
    \item \textbf{Método por parametrización:} se resuelven las restricciones y se escriben las variables originales en función de una o más variables libres

    \item \textbf{Método matricial:} se busca una matriz \(B\) cuyas columnas describan todas las direcciones permitidas por las restricciones, y se estudia la nueva matriz restringida
    \[
    M = B^T A B
    \]
\end{itemize}

En ambos casos, el objetivo es transformar la forma original en una nueva forma cuadrática más pequeña, escrita solamente en términos de las variables libres. Luego se estudia el signo de esa forma restringida.
...
\noindent\textbf{Verificación mediante autovalores.}\\
La forma restringida es
\[
\psi(y)=2y^2
\]

Matricialmente, esto se puede escribir como
\[
\psi(y)=y^T M y
\]
donde
...
Como \(M\) es una matriz de \(1\times 1\), su único autovalor es
\[
\lambda_1=2
\]

Como el único autovalor de la matriz restringida es positivo, la forma restringida es positiva definida.

\medskip
\subsection*{Método matricial: matriz de direcciones permitidas}
...
\paragraph*{1. Matriz \(A\) y matriz de restricciones \(C\).}
...
\paragraph*{2. Matriz de direcciones permitidas.}

Buscamos describir todos los vectores \(x\) que cumplen las restricciones. Partimos de
\[
\begin{cases}
x_1+x_2-x_3=0\\[4pt]
-x_1+x_3=0
...
Entonces todo vector que cumple las restricciones tiene la forma
\[
x=
\begin{pmatrix}
x_1\\
...
Sacando factor común \(x_1\), obtenemos
\[
x=
x_1
\begin{pmatrix}
...
Si llamamos \(y=x_1\), podemos escribir
\[
x=
\begin{pmatrix}
1\\
...
Por lo tanto, definimos la matriz de direcciones permitidas como
\[
B=
\begin{pmatrix}
1\\
...
La matriz \(B\) contiene las direcciones que cumplen las restricciones. En este caso hay una sola dirección permitida, por eso \(B\) tiene una sola columna.

\paragraph*{3. Forma cuadrática restringida.}

Como todo vector que cumple las restricciones se puede escribir como
\[
x=By
\]
sustituimos esto en la forma cuadrática original:
\[
\varphi(x)=x^T A x
\]
...
queda
\[
\varphi(By)=y^T B^T A B y
\]

Por eso, la matriz de la forma cuadrática restringida es
\[
M=B^T A B
\]
...
y la forma cuadrática restringida queda
\[
\psi(y)=y^T M y
\]
\[
...
\paragraph*{4. Verificación mediante autovalores.}

Como la matriz restringida es
\[
M=(2)
\]
...
\lambda_1>0
\]
la forma restringida es positiva definida.

\paragraph*{5. Conclusión.}

La forma cuadrática original, restringida a los vectores que cumplen las restricciones, queda reducida a
\[
\psi(y)=2y^2
\]
...
\[
2y^2>0
\quad \text{para todo } y\neq 0
\]
concluimos que la forma cuadrática \(\varphi\) es \emph{positiva definida} sobre las direcciones permitidas por las restricciones.
...
\subsection*{Otro ejemplo}
\[
\varphi(x_1,x_2,x_3)=x_1^2+2x_2^2-x_3^2
\]
...
\subsubsection*{Método matricial con matriz de direcciones permitidas}

La forma cuadrática puede escribirse como
\[
\varphi(x)=x^T A x
\]
donde
...
Entonces todo vector que cumple la restricción tiene la forma
\[
x=
\begin{pmatrix}
x_1\\
...
Por lo tanto, la matriz de direcciones permitidas es
\[
B=
\begin{pmatrix}
1 & 0\\[4pt]
...
Ahora sustituimos en la forma cuadrática:
\[
\varphi(x)=x^T A x
\]
\[
...
Por lo tanto, la matriz restringida es
\[
M=B^T A B
\]
...
La forma restringida queda
\[
\psi(y)=y^T M y
\]
\[
\psi(y_1,y_2)=y_1^2+2y_2^2
\]
...
Como ambos autovalores son positivos, la forma restringida es definida positiva.

{\color{teal}
\textbf{Conclusión:} la forma cuadrática \(\varphi\) es definida positiva sobre las direcciones permitidas por la restricción \(x_3=0\)}
\newpage
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    axis lines=middle,
    xlabel={$x$}, ylabel={$y$},
    title={Cuasiconvexa (no estricta) y cuasic\'oncava (no estricta):  $g(x,y)=x+y$},
    xmin=0, xmax=2.5,
    ymin=0, ymax=2.5,
    domain=0:2,
    samples=2,
...
\[
\color{teal}
f(x,y)=x\,y\text{ es estrictamente cuasicóncava en }A 
\]
...
\begin{itemize}
  ...
  \item \textbf{Operaciones y composición:}
    \begin{itemize}
      ...
      \item La estricta cuasiconcavidad/cuasiconvexidad se conserva bajo composición con funciones estrictamente crecientes
      \item Además, sea
      \[
        h(x) = (g\circ f)(x)
      \]
...
Los menores principales no siguen un patrón ya que por ejemplo $6x$, puede ser negativo o positivo. Lo mismo con $-6y$ y $|H|$. No podemos afirmar que tenemos un óptimo global.
...
% --- Pegar en el preámbulo ---
% \usepackage{xcolor}
% \usepackage{tikz}
% \usetikzlibrary{arrows.meta, positioning, fit, shapes.geometric, calc}
% \definecolor{teal}{RGB}{0,128,128}

% --- Diagrama de flujo: esquema de análisis sin restricciones ---
\begin{tikzpicture}[
  node distance = 8mm and 12mm,
  >=Stealth,
  font=\small,
  block/.style      = {draw, rounded corners, align=center, inner sep=6pt, fill=teal!5},
  outcome/.style    = {draw, rounded corners, align=center, inner sep=6pt},
  titlebox/.style   = {draw, very thick, rounded corners, fill=teal!10, align=center, inner sep=6pt},
  arrow/.style      = {-{Stealth[length=2mm,width=2mm]}, thick}
]

% Título
\node (title) [titlebox] {\textbf{Esquema de análisis (sin restricciones)}};

% CPO
\node (cpo) [block, below=of title] {CPO: $\nabla F(x^*)=\mathbf{0}$\\(puntos críticos)};

% CLASIFICACIÓN LOCAL
\node (local) [block, below=of cpo] {Evaluar Hessiano en $x^*$: $H(x^*)$};

% Resultados locales (solo mínimo y máximo)
\node (minloc) [outcome, below left=10mm and -4mm of local, xshift=-18mm]
  {$H(x^*) \succ 0$\\$\Rightarrow$ \textbf{\color{teal}mínimo local}};
\node (maxloc) [outcome, below right=10mm and -4mm of local, xshift=18mm]
  {$H(x^*) \prec 0$\\$\Rightarrow$ \textbf{\color{teal}máximo local}};

% Flechas zona local
\draw[arrow] (cpo) -- (local);
\draw[arrow] (local) -- (minloc);
\draw[arrow] (local) -- (maxloc);

% Caja englobando la parte local
\node[draw, dashed, rounded corners,
      fit=(local)(minloc)(maxloc),
      label={[teal,yshift=18mm]above:\strut Clasificación \textit{local}}] (localbox) {};

% ========================
% CLASIFICACIÓN GLOBAL (centrada y simétrica)
% ========================

% Nodo de arranque global
\node (global) [block, below=18mm of localbox.south]
  {Analizar $H(x)$ en el dominio (sin evaluar en un punto)};

% Punto medio para centrar columnas globales
\coordinate (globmid) at ($(global.south)+(0,-8mm)$);

% Columna izquierda (mínimos), anclada a la izquierda del punto medio
\node (minglob) [outcome, text width=42mm, anchor=north east]
  at ($(globmid)+(-10mm,0)$)
  {$H(x)\succeq 0\ \forall x$\\$\Rightarrow$ $F$ convexa\\$\Rightarrow$ \textbf{\color{teal}mínimo global}};
\node (minuniq) [outcome, text width=42mm, below=6mm of minglob]
  {$H(x)\succ 0\ \forall x$\\$\Rightarrow$ $F$ estrictamente convexa\\$\Rightarrow$ \textbf{\color{teal}mínimo global y único}};

% Columna derecha (máximos), anclada a la derecha del punto medio
\node (maxglob) [outcome, text width=42mm, anchor=north west]
  at ($(globmid)+(10mm,0)$)
  {$H(x)\preceq 0\ \forall x$\\$\Rightarrow$ $F$ cóncava\\$\Rightarrow$ \textbf{\color{teal}máximo global}};
\node (maxuniq) [outcome, text width=42mm, below=6mm of maxglob]
  {$H(x)\prec 0\ \forall x$\\$\Rightarrow$ $F$ estrictamente cóncava\\$\Rightarrow$ \textbf{\color{teal}máximo global y único}};

% Flechas zona global
\draw[arrow] (localbox.south) -- (global);
\draw[arrow] (global) -- (minglob);
\draw[arrow] (global) -- (minuniq);
\draw[arrow] (global) -- (maxglob);
\draw[arrow] (global) -- (maxuniq);

% Caja englobando la parte global
\node[draw, dashed, rounded corners,
      fit=(global)(minglob)(minuniq)(maxglob)(maxuniq),
      label={[teal,yshift=6mm]above:\strut Clasificación \textit{global}}] (globalbox) {};
